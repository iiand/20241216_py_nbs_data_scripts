...
def query_isni_database(name, date, username, password):
    try:
       # Construct the ISNI query URL
        ...
        # Replace ', ' with '%2C+', 'period space' with '%2E+', and spaces with '+'
        name_encoded = name.replace(', ', '%2C+').replace('. ', '%2E+').replace(' ', '+')

        query = f"?query=pica.nw+%3D+{name_encoded}%2C%22+and+pica.dti+%3D+{date}%22&version=1.1&operation=searchRetrieve&recordSchema=isni-e"
        url = base_url + query
            ...
            if response.text.strip():  # Check if the response is not empty
                ...
                if best_record is not None:
                    ...
                    if match:
                        ...
                        # Find personal name elements for sources 'NLS' or 'LCNACO'
                        personal_names = []
                        for person in re.findall(r'<personalName>.*?</personalName>', ET.tostring(best_record, encoding='unicode'), re.DOTALL):
                            source_match = re.search(r'<source>(NLS|LCNACO)<\/source>', person)
                            if source_match:
                                source = source_match.group(1)
                                name_parts = re.findall(r'<(forename|surname|nameTitle)>(.*?)<\/\1>', person)
                                personal_name = f"{source}|{' '.join([part[1] for part in name_parts])}"
                                personal_names.append(personal_name)

                        return isni_uri, wikidata_url, personal_names
                    else:
                        print("No WKP source found in ISNI record.")
                else:
                    print(f"No records found in initial ISNI query: {url}")

                    # If no results, try with 'na' and 'aw' search elements
                    #query = f"?query=pica.na+%3D+%22{name.replace(', ', '%2C+').replace('. ', '%2E+').replace(' ', '+')}%2C%22+and+pica.aw+%3D+%22{date}&version=1.1&operation=searchRetrieve&recordSchema=isni-e"
                    #url = base_url + query
                    #response = requests.get(url)
                        ...
                        if response.text.strip():
                            ...
                            if best_record is not None:
                                ...
                                if match:
                                    ...
                                    print(f"No WKP source found in ISNI record (second query): {url}")
                            else:
                                print(f"No records found in second ISNI query: {url}")
                        else:
                            print(f"Empty response from second ISNI query: {url}")
                    else:
                        print(f"Error: Received status code {response.status_code} from second ISNI query: {url}")
            else:
                print(f"Empty response from initial ISNI query: {url}")
        else:
            print(f"Error: Received status code {response.status_code} from initial ISNI query: {url}")
...
for record in tqdm(root.findall('record'), desc="Processing records"):
    ...
    if has_isni_subfield and isni_url_present:
        ...
        _, wikidata_url, personal_names = query_isni_database(name, date, username, password)
        ...
        if personal_names:
            personal_name = personal_names[0]  # Use the first personal name found
            print(f"Adding personal name: {personal_name}")
            new_subfield = ET.Element('subfield', {'code': '9'})
            new_subfield.text = personal_name
            datafields[0].append(new_subfield)
        ...
        # Query the ISNI database using the extracted values (name and date)
        isni_url, wikidata_url, personal_names = query_isni_database(name, date, username, password)
...
tree.write('C:/Users/ssa041rs/Desktop/202412_alma_xml/20241216_py_nbs_data_scripts/results/isni_wiki/isni_wiki_v4_20241220_nbs_id_scot_author_matched_bibrecs.xml')

print("The updated records have been written to 'isni_wiki_isni_wiki_v4_20241220_nbs_id_scot_author_matched_bibrecs.xml'.")